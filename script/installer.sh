...
apt-get update
apt-get install software-properties-common python3 python3-dev libmariadbclient-dev build-essential python3-setuptools install libssl-dev git
easy_install-3.4 install pip
apt-key adv --recv-keys --keyserver hkp://keyserver.ubuntu.com:80 0xcbcb082a1bb943db
add-apt-repository 'deb [arch=amd64,i386,ppc64el] http://kodeterbuka.beritagar.id/mariadb/repo/10.1/ubuntu trusty main'
add-apt-repository ppa:nginx/stable
apt-get update && apt-get install mariadb-server nginx
...
echo "$(tput setaf ) $(tput bold) $NOTIF_4"
cd /server/file/
